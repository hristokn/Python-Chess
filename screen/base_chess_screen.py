...
class BaseChessScreen(Screen, ABC):
    def __init__(self, mouse: Mouse, image_library: ImageLibrary, event_announcer: EventAnnouncer,
                 sound_player: SoundPlayer, color: Color, board_controller):
        ...
        for i in range(8):
            ...
            label_file = Text(self.draw_x1 + 80 + step * i, self.draw_y1 + 594,
                              ...
                              1, self.image_library, labels_file[i])
    ...
    def receive_event(self, event):
        if event.type == CustomEvent.FINISHED_GAME.value and self.game_end_popup is None:
