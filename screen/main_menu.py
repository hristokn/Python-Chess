...
from custom_events import post_event, CustomEvent
...
class MainMenu(Screen):
    def __init__(self, mouse, image_library, event_announcer: EventAnnouncer):
        ...
        self.start_practice_button = TextButton(500, 400, 1, self.image_library, 'Practice', self.start_practice_game)
        self.add_element(self.start_practice_button)
        self.mouse.register_button_observer(self.start_practice_button)

        color_button_x = self.start_game_button.draw_x1 + self.start_game_button.width() + 30
        color_button_y = self.start_game_button.draw_y1 + (self.start_game_button.height() - self.image_library['button_white'].get_height()) / 2
    ...
    def start_chess_game(self):
        post_event(CustomEvent.CHANGE_SCREEN, screen_name='chess_game', color=self.color_button.get_value())
    
    def start_practice_game(self):
        post_event(CustomEvent.CHANGE_SCREEN, screen_name='practice_game', color=self.color_button.get_value())
